...
export default function ItemDetailScreen() {
  ...
  const item = useMemo(
    () => data?.items.find((i) => i.id === Number(id)),
    [data, id],
  );
  const steps: WizardStep[] = useMemo(() => {
    ...
    return data.modifiers
      ...
      .map((m) => ({
        modifier: m,
        options: data.modifierOptions.filter(
          (o) => o.modifierId === m.modifierId,
        ),
      }));
  }, [data, item]);
  const unitPrice = useMemo(() => {
    if (!item) return 0;
    const delta = Object.values(selections)
      .flat()
      .reduce(
        (sum, oid) =>
          sum +
          (data?.modifierOptions.find((o) => o.id === oid)?.priceDelta ?? 0),
        0,
      );
    return item.basePrice + delta;
  }, [item, selections, data]);
  const toggleOption = useCallback(
    (modifier: MenuModifier, optionId: number) => {
      setSelections((prev) => {
        const cur = prev[modifier.modifierId] ?? [];
        if (modifier.modifierType === 'single')
          return { ...prev, [modifier.modifierId]: [optionId] };
        if (cur.includes(optionId))
          return {
            ...prev,
            [modifier.modifierId]: cur.filter((x) => x !== optionId),
          };
        if (modifier.maxChoices && cur.length >= modifier.maxChoices) {
          Alert.alert(
            'Limit Reached',
            `You can choose up to ${modifier.maxChoices}.`,
          );
          return prev;
        }
  ...
  const goNext = useCallback(() => {
    ...
    if (
      step?.modifier.required &&
      (selections[step.modifier.modifierId] ?? []).length === 0
    ) {
      Alert.alert('Required', `Please choose "${step.modifier.modifierName}".`);
      return;
    }
    setStepIndex((i) => i + 1);
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        {(stepIndex === 0 || onReview) && (
          <View style={s.itemHeader}>
            {item.imageUrl ? (
              ...
              <View style={[s.heroImg, s.heroImgFallback]}>
        ...
        {onReview && (
          <ReviewScreen
            ...
            allOptions={data?.modifierOptions ?? []}
            unitPrice={unitPrice}
            qty={qty}
            onChangeQty={setQty}
      ...
      <View style={s.footer}>
        ...
        {!onReview ? (
          <View style={s.nextGroup}>
            {!steps[stepIndex]?.modifier.required && (
              <TouchableOpacity
                style={s.skipPill}
  ...
  return (
    <View style={ws.wrap}>
      <View style={ws.header}>
        <Text style={ws.stepNum}>
          Step {stepNumber} of {totalSteps}
        </Text>
        <View style={ws.chip}>
          <Text style={ws.chipText}>
            {icon} {modifier.category}
          </Text>
        </View>
      </View>
      <Text style={ws.title}>{modifier.modifierName}</Text>
      {!modifier.required && (
        <Text style={ws.hint}>Optional — tap Skip to continue</Text>
      )}
      {modifier.modifierType === 'multiple' && modifier.maxChoices && (
        <Text style={ws.hint}>Choose up to {modifier.maxChoices}</Text>
      )}
      {modifier.modifierType === 'single' && (
        <Text style={ws.hint}>Choose one</Text>
      )}
      <View style={ws.list}>
        {options.map((opt) => {
          ...
          return (
            <TouchableOpacity
              key={opt.id}
              style={[ws.option, sel && ws.optionSel]}
              onPress={() => onToggle(opt.id)}
              activeOpacity={0.8}
            >
              <View style={[ws.indicator, sel && ws.indicatorSel]}>
                {sel && <View style={ws.indicatorDot} />}
              </View>
              ...
              {opt.priceDelta !== 0 && (
                <Text
                  style={[
                    ws.delta,
                    opt.priceDelta > 0 ? ws.deltaPos : ws.deltaNeg,
                  ]}
                >
                  {opt.priceDelta > 0 ? '+' : ''}$
    ...
    <View style={rv.wrap}>
        ...
        return (
          <View key={step.modifier.modifierId} style={rv.row}>
            <View style={{ flex: 1 }}>
              ...
              {opts.length > 0 ? (
                opts.map((o) => (
                  <Text key={o.id} style={rv.rowVal}>
                    ✓ {o.name}
                  </Text>
                ))
              ) : (
                <Text style={rv.rowNone}>None selected</Text>
              )}
            </View>
            <TouchableOpacity
              onPress={() => onEditStep(i)}
              hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }}
            >
              <Text style={rv.edit}>Edit</Text>
            </TouchableOpacity>
          </View>
        );
      })}
      ...
      <View style={rv.priceCard}>
        <View style={rv.priceRow}>
          <Text style={rv.priceLabel}>Price per item</Text>
          <Text style={rv.priceVal}>${(unitPrice / 100).toFixed(2)}</Text>
        </View>
        <View style={[rv.priceRow, { marginTop: 6 }]}>
          <Text style={rv.totalLabel}>Subtotal</Text>
          <Text style={rv.totalVal}>
...
const ws = StyleSheet.create({
  ...
  optionSel: { borderColor: '#3b82f6', backgroundColor: '#eff6ff' },
  indicator: {
    width: 22,
    height: 22,
    borderRadius: 11,
  ...
  indicatorSel: { borderColor: '#3b82f6' },
  indicatorDot: {
    width: 12,
    height: 12,
    borderRadius: 6,
  ...
  optName: { flex: 1, fontSize: 15, fontWeight: '500', color: '#374151' },
  optNameSel: { color: '#1d4ed8', fontWeight: '700' },
  delta: { fontSize: 13, fontWeight: '600' },
  deltaPos: { color: '#6b7280' },
  deltaNeg: { color: '#059669' },
});
const rv = StyleSheet.create({
  ...
  rowVal: { fontSize: 14, color: '#374151' },
  rowNone: { fontSize: 14, color: '#d1d5db', fontStyle: 'italic' },
  edit: { fontSize: 13, color: '#3b82f6', fontWeight: '700' },
  qtyRow: {
    backgroundColor: '#fff',
    borderRadius: 12,
    padding: 14,
  ...
  qtyCtrl: { flexDirection: 'row', alignItems: 'center', gap: 18 },
  qtyBtn: {
    width: 36,
    height: 36,
    borderRadius: 18,
  ...
  priceVal: { fontSize: 13, color: '#374151' },
...
const s = StyleSheet.create({
  ...
  itemHeader: { backgroundColor: '#fff', marginBottom: 4 },
  ...
  heroDesc: { fontSize: 14, color: '#6b7280', marginTop: 4, lineHeight: 20 },
  footer: {
    backgroundColor: '#fff',
    borderTopWidth: 1,
  ...
  backPill: { paddingHorizontal: 14, paddingVertical: 12 },
  backPillText: { fontSize: 15, color: '#6b7280', fontWeight: '600' },
  nextGroup: { flex: 1, flexDirection: 'row', gap: 8, alignItems: 'center' },
  skipPill: { paddingHorizontal: 14, paddingVertical: 12 },
  skipPillText: { fontSize: 15, color: '#9ca3af', fontWeight: '600' },
  nextBtn: {
    flex: 1,
    backgroundColor: '#111827',
    borderRadius: 12,
