...
function promoLabel(p: Promotion): string {
  ...
  if (p.type === 'buy_x_get_y')
    return `Buy ${p.triggerQty} Get ${p.rewardQty} ${p.value === 100 ? 'FREE' : `${p.value}% off`}`;
  ...
  if (isLoading)
    return (
      <View style={s.center}>
        ...
        <Text style={s.loadTxt}>Loading deals…</Text>
      </View>
    );
          ...
          <View style={s.section}>
            ...
            {autoPromos.map((p) => (
              <View key={p.id} style={s.cardGreen}>
                <View style={s.pRow}>
                  <Text style={s.pIcon}>{ICONS[p.type] ?? '🏷️'}</Text>
                  <View style={{ flex: 1 }}>
                    <Text style={s.pName}>{p.name}</Text>
                    <Text style={s.pVal}>{promoLabel(p)}</Text>
                  </View>
                ...
                {p.minOrderTotal > 0 && (
                  <Text style={s.pMin}>
                    Min order: ${(p.minOrderTotal / 100).toFixed(2)}
                  </Text>
                )}
              </View>
            ))}
            ...
            {codePromos.map((p) => (
              <View key={p.id} style={s.cardYellow}>
                ...
                <View style={s.codePill}>
                  <Text style={s.codeTxt}>{p.promotionCode}</Text>
                </View>
              </View>
            ))}
          </View>
              ...
              {featured.map((item) => (
                ...
                >
                  ...
                  <Text style={s.itemPrice}>
                    from ${(item.basePrice / 100).toFixed(2)}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
...
const s = StyleSheet.create({
  ...
  secTitle: { fontSize: 18, fontWeight: '700', color: '#111827' },
  secSub: { fontSize: 12, color: '#9ca3af', marginTop: -6 },
  cardGreen: {
  ...
  pVal: { fontSize: 13, fontWeight: '600', color: '#059669', marginTop: 2 },
  pDesc: { fontSize: 12, color: '#374151' },
  pMin: { fontSize: 11, color: '#9ca3af' },
  autoBadge: {
  ...
  noDeals: { padding: 28, alignItems: 'center' },
  noDealsT: { color: '#9ca3af', fontSize: 13 },
  ...
  menuBtnTxt: { color: '#fff', fontWeight: '700', fontSize: 16 },
  cartBar: {
    position: 'absolute',
    bottom: 16,
    left: 16,
  ...
  cartBarTxt: { color: '#fff', fontWeight: '700', fontSize: 15 },
