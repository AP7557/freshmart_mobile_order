...
> = {
  percent: {
    ...
    icon: '🏷️',
    desc: 'Percentage discount off items or the whole order',
  },
  fixed: {
    label: '$ Off',
  ...
  item: {
    ...
    icon: '🛒',
    desc: 'Discount on specific menu items',
  },
  buy_x_get_y: {
    label: 'Buy X Get Y',
    color: 'bg-orange-100 text-orange-800',
    icon: '🎁',
    desc: 'Buy X items → get Y items free or discounted',
  },
  bundle: {
    label: 'Bundle Deal',
...
function valueLabel(p: Promotion) {
  ...
  if (p.type === 'buy_x_get_y') {
    const disc =
      p.value === 100 ? 'FREE' : p.value > 0 ? `${p.value}% off` : 'discount';
...
function isExpired(p: Promotion) {
  return new Date(p.endAt) < new Date();
}
function isScheduled(p: Promotion) {
...
export default function PromotionsPage() {
  ...
  const live = promos.filter(
    (p) => p.isActive && !isExpired(p) && !isScheduled(p),
  ).length;
  ...
  return (
    <div className='space-y-6'>
      ...
      {!loading && (
        <div className='grid grid-cols-3 gap-3'>
          {[
            ...
            {
              label: 'Scheduled',
              value: promos.filter((p) => p.isActive && isScheduled(p)).length,
        ...
        >
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            <label className='flex flex-col gap-1 text-sm'>
              ...
              <input
                {...register('name')}
                placeholder='e.g. "Summer Sub Deal"'
              ...
              <label className='flex flex-col gap-1 text-sm'>
                ...
                <div className='relative'>
                  <input
                    type='number'
                    {...register('value')}
                    placeholder={watchedType === 'fixed' ? '500 = $5.00' : '10'}
            ...
            {watchedType === 'buy_x_get_y' && (
              <>
                ...
                <label className='flex flex-col gap-1 text-sm'>
                  ...
                  />
                  <span className='text-xs text-gray-400'>
                    Reward items that get discounted
                  </span>
                </label>
                <label className='flex flex-col gap-1 text-sm'>
                  <span className='font-medium text-gray-700'>
          ...
          <div className='border border-gray-100 rounded-xl p-4 space-y-3 bg-gray-50'>
            <div className='flex items-center justify-between'>
              ...
              <span className='text-xs text-gray-400'>
                Leave blank → auto-applies at checkout
              </span>
            </div>
            <div className='flex gap-3 items-center'>
              <input
                {...register('promotionCode')}
                placeholder='e.g. SAVE10, HALFOFF'
              ...
              <Controller
                ...
                render={({ field }) => (
                  <ItemPicker
                    label={
                      watchedType === 'item'
                        ? 'Items that get the discount (required)'
                        : 'Limit to specific items (leave empty = whole order)'
                    }
            ...
            <div className='border border-gray-100 rounded-xl p-4 bg-gray-50 space-y-4'>
              <p className='text-sm font-semibold text-gray-700'>
                ...
                <span className='ml-2 text-xs font-normal text-gray-400'>
                  Buy {watchedTriggerQty ?? 1} → get {watchedRewardQty ?? 1} at{' '}
                  {watchedValue === 100 ? 'FREE' : `${watchedValue}% off`}
                </span>
              </p>
              <Controller
                name='triggerItemIds'
              ...
              <div className='bg-orange-50 border border-orange-100 rounded-lg px-3 py-2 text-xs text-orange-700'>
                💡 Example: Select "Half Sub" as trigger (qty 1) + "Roll Sub" as
                reward (qty 1) at 100% off → buy any Half Sub, get a Roll Sub
                free.
              </div>
            </div>
          )}
          ...
          {watchedType === 'bundle' && (
            <div className='border border-gray-100 rounded-xl p-4 bg-gray-50 space-y-2'>
              ...
              <div className='bg-rose-50 border border-rose-100 rounded-lg px-3 py-2 text-xs text-rose-700'>
                💡 Example: Add "Half Sub" + "Drink" + "Chips" → {watchedValue}%
                off when all 3 are in cart.
              </div>
            </div>
          )}
      ...
      <div className='flex flex-wrap gap-1.5'>
        {[
          ['all', 'All'],
          ...Object.entries(TYPE_META).map(([t, m]) => [t, m.label]),
        ].map(([val, label]) => (
          <button
            key={val}
      ...
      {loading ? (
        ...
        <div className='text-center py-16 text-gray-400'>
          ...
          <p className='text-sm mt-1'>
            Create code-based, product, or bundle deals
          </p>
        </div>
      ) : (
        <div className='space-y-3'>
          {filtered.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`bg-white border rounded-xl shadow-sm overflow-hidden ${!p.isActive || isExpired(p) ? 'opacity-60' : ''}`}
              >
                <div className='flex items-start gap-3 px-4 py-3'>
                  ...
                  <div className='flex-1 min-w-0 space-y-1.5'>
                    ...
                    <p className='text-xs text-gray-400'>
                      ...
                      {p.minOrderTotal > 0 &&
                        ` · min $${(p.minOrderTotal / 100).toFixed(2)}`}
                    </p>
                  </div>
                  ...
                  <div className='border-t border-gray-100 bg-gray-50 px-4 py-3 text-xs space-y-2 text-gray-600'>
                    {p.type === 'buy_x_get_y' && (
                      <div className='space-y-1'>
                        <p>
                          <strong>Buy:</strong> {p.triggerQty} item
                          {p.triggerQty > 1 ? 's' : ''}{' '}
                          {p.triggerItemIds?.length
                            ? `from: ${p.triggerItemIds.map((id) => allItems.find((i) => i.id === id)?.name ?? `#${id}`).join(', ')}`
                            : '(any item)'}
                        </p>
                        <p>
                          <strong>Get:</strong> {p.rewardQty} item
                          {p.rewardQty > 1 ? 's' : ''}{' '}
                          {p.rewardItemIds?.length
                            ? `from: ${p.rewardItemIds.map((id) => allItems.find((i) => i.id === id)?.name ?? `#${id}`).join(', ')}`
                            : '(cheapest qualifying)'}{' '}
                          at {p.value === 100 ? 'FREE' : `${p.value}% off`}
                        </p>
                      </div>
                    )}
                    {p.type === 'bundle' && (
