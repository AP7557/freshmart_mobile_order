...
export async function PATCH(
  ...
  try {
    ...
    // Update the modifier row if any modifier fields changed
    ...
    if (options) {
      ...
      opts = await db
        ...
        .values(options.map((o) => ({ ...o, modifierId: modId })))
        .returning();
    } else {
      opts = await db
        .select()
        .from(modifierOptions)
        .where(eq(modifierOptions.modifierId, modId));
    }
...
export async function DELETE(
  ...
  try {
    ...
    const modId = parseInt(id, 10);
    if (isNaN(modId)) return err("Invalid modifier id");
    // modifierOptions cascade delete via FK
