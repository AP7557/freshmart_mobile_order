...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.insert(itemModifiers).values({ modifierId: modId, itemId: parsed.data.itemId }).onConflictDoNothing();
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = BodySchema.safeParse(await req.json());
    if (!parsed.success) return err(parsed.error.issues[0].message);
    await db.delete(itemModifiers).where(and(eq(itemModifiers.modifierId, modId), eq(itemModifiers.itemId, parsed.data.itemId)));
